...
(Если приложение нацелено на серьёзные знакомства - HeartSync. Для лёгкого общения и флирта — FlirtFinder или BlinkDate.)
Выбор названия зависит от цели приложения
...
- Пока существует проблема в виде дублирования реакций на анкеты пользователей в третьем окне и не показываются эти самые пропущенные анкеты - пока не знаю как это поправить. После решения пролем с листанием анкет можно вернуться на пропущенную анкету и поставить реакцию, но как это поправить без таких костылей я пока не знаю.
- Нет возможности листать анкеты вперед назад - это вроде исправлено
