...
## Функционал проекта
 ...
 - [х] Модерация переписок и анкет, реализовано только возможность скинуть сообщение в техподдержку и написать жалобу на пользователя, нарушившего правила использования приложения
...
**Доработка функционала**
 ...
 - **Доделать констектное меню на пятой странице:**
	1. Починить и допилить кнопку удалить так как в ней возникают ошибки. В удалении почему-то путается id сообщения. Ну точно не знаю как это рабоатет, но если закрыть приложение и снова открыть, то все рабоатет корректно.
	2. Кнопка ответить не всегда корректно работает и выделяет серым цветом уже другие сообщения, которые не нужно выделять.
	3. Сделать возможность для меню редактирования сообщения отправлять сообщение нажатием на enter и переносить строку с помощью shift + enter. Я не знаю как это сделать, так как его нет в ui файле и делать обработку нажатия клавиш для него бесполезно.
	4. Реализовать функцию - Закрепить сообщение
	5. Реализовать функцию - Пожаловаться на сообщение
...
Закрепить сообщение, Пожаловаться на сообщение - дополнительные полезные операции для контекстного меню, они еще не рализованы даже в плане работы напрямую с бд (опционально и планируется в будущем)
